...
//遍历输出所有区块信息
func (blc *Blockchain) Printchain(nodeID string) {
	//生成迭代器
...
func (blockchain *Blockchain) MineNewBlock(from, to, amount []string,nodeID string) {
	...
	for i := 0; i < len(from); i++ {
		...
		//生成每一笔tx交易
		tx := NewSimpleTransaction(from[i], to[i], value, utxoSet, txs,nodeID)
		txs = append(txs, tx)
	}
	//创建新区块之前验证签名有效性
	//遍历每一笔交易签名的有效性
...
func (bc *Blockchain) FindUnspentUTXOMap() map[string]*TxOutputs {
	...
	for {
		...
		//从后往前遍历区块
		for i := len(block.Txs) - 1; i >= 0; i-- {
			tx := block.Txs[i] //拿到每一个tx
			//创建一个空的TxOutputs对象，用来装所有找到的UTXO
			txOutputs := &TxOutputs{[]*UTXO{}}

			//如果是coinbase交易就不需要添加
			if !tx.IsCoinBaseTransaction() {
				//1.先遍历Vins中的tx中的input，存到map中
				for _, input := range tx.Vins {
					//转换txID
			...
			//拿到当前要遍历的TxID,一会遍历所有的output的时候需要用到
			txIDStr := hex.EncodeToString([]byte(tx.TxID))
			//2.再遍历Vouts中的output,
		outputLoop:
			for index, output := range tx.Vouts {
				//拿到该txid对应的map中的所有的inputs，都是已经花费的，一会要对比用
				inputs := spentedMap[txIDStr]
				if len(spentedMap) > 0 { //如果map有数据
					isSpent := false //设置已经花费标记为false,代表暂时未花费
					//遍历所有的取到的inputs中的记录
					for _, input := range inputs {
						//对比花钱人用的
						inputPubKeyHash := PubKeyHash(input.PublicKey)
						//判断如果input中的公钥哈希和output中的一样 并且 input中的下标和output中的下标一样，就说明已经花掉了
						if bytes.Compare(inputPubKeyHash, output.PubKeyHash) == 0 && input.Vout == index {
							isSpent = true      //标记为已花费
							continue outputLoop //当前的output就查了，查下一个
						}
					}
					//如果查询所有的output都没有被花费，就存上
					if isSpent == false {
						//生成UTXO
						utxo := &UTXO{tx.TxID, index, output}
						//将生成的UTXO加到txOutputs中
						txOutputs.UTXOs = append(txOutputs.UTXOs, utxo)
					}

				} else { //如果map中没数据，就说明都是未花费的，直接存就行了
					utxo := &UTXO{tx.TxID, index, output}
					//将生成的UTXO加到txOutputs中
					txOutputs.UTXOs = append(txOutputs.UTXOs, utxo)
				}
			}
			//将当前的这个tx中，获取到的全部未花费的txOutputs,都存到未花费map中
		...
		//判断for结束
		bigInt := new(big.Int)
		bigInt.SetBytes(block.PrevBlockHash) //遍历到创世区块就退出循环
		if big.NewInt(0).Cmp(bigInt) == 0 {
			break
		}
	}
...
//查询自己的数据表，将查到的hash拼接并返回拼接好的hash数组
...
func (bc *Blockchain) AddBlock(block *Block)  {
	err:=bc.DB.Update(func(tx *bolt.Tx) error {
		...
		if b!=nil{
			...
			//更新最新的hash
			lastBlockBytes:=b.Get(b.Get([]byte("l"))) //拿到最新区块byte
			//反序列化拿到最新区块
			lastBlock:=DeserializeBlock(lastBlockBytes)
			//如果我当前数据库中的区块高度小于要添加的区块高度，就更新最新hash值

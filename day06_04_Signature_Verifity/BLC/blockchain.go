...
func (blockchain *Blockchain) MineNewBlock(from []string, to []string, amount []string) {
	...
	//遍历每一笔交易签名的有效性
	for _,tx:=range txs{
		if blockchain.VerifityTransaction(tx)==false{
			log.Panic("数字签名验证失败....")
		}
	}
	//用来存储拿到的区块
...
//签名
func (bc *Blockchain) SignTransaction(tx *Transaction, prevateKey ecdsa.PrivateKey) {
	...
	for _, input := range tx.Vins {
		...
		//跟tx中的每个input对应txID，获取要转账的的output所在的Transaction，存入到map[input.TxID]-->Tx
